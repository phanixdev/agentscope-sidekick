...
function Sidebar({ view, setView, workspace, open, close, runs, filters, setFilters, alertCount, preview }) {
  const nav = [
  ...
  return <aside className={`sidebar ${open ? "open" : ""}`}>
    ...
    {view === "runs" && <div className="filters">
      ...
      <div className="filter-summary"><RadioTower size={14}/><span>{preview ? "Deterministic judge dataset" : "Workspace telemetry · last 24 hours"}</span></div>
    </div>}
  </aside>;
}
...
function Inspector({ run, openNote, openProof, notify, alertContext, onRemediate, remediationBusy, remediationError }) {
  ...
  return <section className="inspector panel">{alertContext && <div className="alert-context"><Bell size={14}/><span>Opened from <strong>{alertContext.name}</strong><small>{alertContext.metric} {alertContext.threshold}</small></span></div>}<div className="inspector-title"><span className={`incident ${tone(run.status)}`}>{run.status === "failed" ? <X size={14}/> : <Check size={14}/>}</span><strong>{run.id}</strong><span>{run.agent}</span><span className={`status ${tone(run.status)}`}><i/>{run.status}</span><time>{run.startTime}</time></div><div className="tabs">{[["explain", "Explain"], ["evidence", "Evidence"], ["compare", "Compare"], ["remediate", "Remediate"], ["details", "Details"], ["metrics", "Metrics"], ["events", "Events"]].map(([id, label]) => <button key={id} className={tab === id ? "active" : ""} onClick={() => setTab(id)}>{label}</button>)}<button className="note" onClick={openNote}><FileText size={15}/>Note</button></div>{tab === "explain" && <Explanation run={run} openProof={openProof}/>} {tab === "evidence" && <div className="evidence-tab"><div className="trace-strip"><span><small>Correlated trace</small><code>{run.traceId}</code></span><button aria-label="Copy trace ID" className="icon-button" onClick={copyTrace}><Copy size={15}/></button></div><div className="provenance-grid"><span><small>Source</small><strong>SigNoz / OpenTelemetry</strong></span><span><small>Captured</small><strong>{formatCapturedAt(run)}</strong></span><span><small>Span ID</small><code>{anomalySpan?.id || "Available in trace query"}</code></span><span><small>Metric query</small><code>{metric.query}</code></span><span><small>Rule ID</small><code>{provenance.ruleId}</code></span><span><small>Rule version</small><code>{provenance.ruleVersion}</code></span><span><small>Decision engine</small><strong>{provenance.engine}</strong></span><span><small>LLM involvement</small><strong>{provenance.model}</strong></span><span><small>Prompt version</small><strong>{provenance.promptVersion}</strong></span><span><small>Evidence contract</small><code>trace + metric + log</code></span></div><EvidenceList run={run}/><button className="secondary full-width" onClick={openProof}><RadioTower size={15}/>Inspect captured SigNoz trace, dashboard, and alerts</button></div>} {tab === "compare" && <Comparison run={run}/>} {tab === "remediate" && <Remediation run={run} onApply={onRemediate} busy={remediationBusy} error={remediationError}/>} {tab === "details" && <div className="detail-grid">{detail.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div>} {tab === "metrics" && <div className="metric-strip">{[["Duration", `${run.latency.toFixed(2)}s`, run.latency > 10 ? "danger" : "ok"], ["Tokens", run.tokens.toLocaleString(), run.tokens > 12000 ? "warn" : "ok"], ["Retrieval", run.retrieval.toFixed(2), run.retrieval < 0.3 ? "danger" : "ok"], ["Cost", `${run.cost.toFixed(3)}`, "ok"]].map(([label, value, kind]) => <div key={label}><span>{label}</span><strong className={kind}>{value}</strong></div>)}</div>} {tab === "events" && <div className="events">{run.logs.map((log) => <div key={log.id || log.time + log.message}><time>{log.time}</time><b className={tone(log.level.toLowerCase())}>{log.level}</b><span>{log.message}</span></div>)}</div>}</section>;
}
...
function Product({ user, preview }) {
  ...
  return <main className="app-shell"><Sidebar view={view} setView={changeView} workspace={workspace} open={mobile} close={() => setMobile(false)} runs={runs} filters={filters} setFilters={setFilters} alertCount={alerts.filter((alert) => alert.enabled).length} preview={preview}/>{mobile && <div className="scrim" onClick={() => setMobile(false)}/>}<div className="workspace"><Topbar user={user} preview={preview} workspace={workspace} openMenu={() => setMobile(true)} menuOpen={mobile} query={query} setQuery={setQuery} openProof={() => setProofOpen(true)} showRuns={() => { setView("runs"); setInvestigation(null); }}/><div className="content">{loading ? <div className="loading" role="status"><LoaderCircle className="spin" size={26}/><span>{loadSlow ? "Telemetry is taking longer than expected..." : "Loading workspace telemetry..."}</span></div> : loadError ? <WorkspaceFailure error={loadError} preview={preview} retry={() => setReloadToken((value) => value + 1)}/> : view === "overview" ? <Overview runs={runs} openRuns={() => { setView("runs"); setInvestigation(null); }} openProof={() => setProofOpen(true)} preview={preview}/> : view === "alerts" ? <AlertsView alerts={alerts} setAlerts={setAlerts} notify={notify} preview={preview} openProof={() => setProofOpen(true)} onInvestigate={openAlertInvestigation} runs={runs}/> : view === "team" ? <TeamView user={user} workspace={workspace} preview={preview}/> : <RunsView runs={runs} setRuns={setRuns} workspace={workspace} notify={notify} preview={preview} query={query} setQuery={setQuery} filters={filters} openProof={() => setProofOpen(true)} targetRunId={investigation?.runId} alertContext={investigation?.alert}/>}</div></div>{proofOpen && <ProofModal close={() => setProofOpen(false)}/>} {toast && <div className={`toast ${toast.kind}`} role="status"><CheckCircle2 size={17}/>{toast.message}</div>}</main>;
}
